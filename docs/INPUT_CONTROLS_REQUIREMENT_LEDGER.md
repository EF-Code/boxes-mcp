...
| 5.2 | Persistent process/session keyed by domain+endpoint | `src/spice.ts`, native session state | spice/native-helper tests | Protocol docs | Endpoint/reconnect live proof | IMPLEMENTED-UNVERIFIED-LIVE | 38561cd, e01dd47, e32ea76; real helper restart recovered connected status, guest reboot endpoint recovery remains unverified |
...
| 5.5 | Timeouts/cancellation/channel lifecycle/reconnect/crash handling | TS pending map; native cancellable/session cleanup | spice lifecycle tests | README troubleshooting | Real disconnect/reconnect | IMPLEMENTED-UNVERIFIED-LIVE | 38561cd, e01dd47, 0b91744, 11dd125, e32ea76; local abort/restart and live helper restart passed, guest reboot reconnect remains unverified |
...
| 7.5 | Round trips and disconnected-agent error | Opt-in harness and manual env gate | `src/integration.test.ts` | README instructions | Both directions; manual agent disconnect | BLOCKED-LIVE | 9b7d77e; Omarchy's Wayland session did not announce clipboard, and stopping its user session agent left the system agent connected for file transfer, so a full disconnected-agent proof was not obtained |
| 8.1 | Confined canonical readable regular source | `src/transfer.ts`, native no-follow check | `src/transfer.test.ts` | Evidence doc | Live source/guest destination | IMPLEMENTED-UNVERIFIED-LIVE | b89910a, 7c97c11, d1b291a; source confinement and 18,682-byte README.md transport exercised, guest destination not inspected |
...
| 8.4 | Success/rejection/cancel/disconnect/timeout/duplicate/crash fixtures | TS/helper lifecycle tests | spice/transfer suites | Matrix | Live transfer failures | IMPLEMENTED-UNVERIFIED-LIVE | b89910a, 38561cd, 11dd125; native cancellation callback lifetime hardened, failure boundaries remain local-only |
| 8.5 | Real transfer and destination semantics | Opt-in `transferFile` test | integration harness | Evidence doc | Disposable Linux guest | IMPLEMENTED-UNVERIFIED-LIVE | 9b7d77e, d1b291a; real 18,682-byte transport completed, but the SPICE API did not expose/verify guest destination semantics |
...
| 10.3 | Exercise all boundaries where available | Screenshot/key/QMP/SPICE/clipboard/transfer/drag tests | Opt-in integration suite | Evidence matrix | Live run and versions | VERIFIED | 9b7d77e, d1b291a, 11dd125; Omarchy run: 7 passed, clipboard and manual disconnect honestly skipped |
