...
suite("opt-in disposable VM integration", () => {
  ...
  it("probes and exercises QMP only when an absolute pointer is observed", async ({ skip }) => {
    ...
    if (capabilities.backends.qmp.state !== "connected") return skip(`QMP not available: ${capabilities.backends.qmp.reason || "unknown"}`);
  ...
  it("exercises SPICE mouse only when all required channels are connected", async ({ skip }) => {
    ...
    if (capabilities.backends.spice.state !== "connected") return skip(`SPICE not connected: ${capabilities.backends.spice.reason || "unknown"}`);
  ...
  it("round-trips clipboard text only when guest-agent clipboard is observed", async ({ skip }) => {
    const vm = requireIntegrationConfiguration();
    const capabilities = await discoverCapabilities(vm, { probeSpice: true });
    if (capabilities.backends.clipboard.state !== "connected") return skip(`Clipboard not connected: ${capabilities.backends.clipboard.reason || "unknown"}`);
  ...
  it("transfers a caller-selected confined file only when file transfer is observed", async ({ skip }) => {
    ...
    if (!sourcePath || !process.env.BOXES_TRANSFER_ROOT) return skip("Set BOXES_TRANSFER_ROOT and BOXES_TEST_SOURCE_PATH for transfer evidence");
    const capabilities = await discoverCapabilities(vm, { probeSpice: true });
    if (capabilities.backends.fileTransfer.state !== "connected") return skip(`File transfer not connected: ${capabilities.backends.fileTransfer.reason || "unknown"}`);
  ...
  it("reports experimental drag evidence without claiming application acceptance", async ({ skip }) => {
    ...
    if (!sourcePath || !process.env.BOXES_TRANSFER_ROOT || !target) return skip("Set transfer-root, source, and valid target variables for drag evidence");
    const capabilities = await discoverCapabilities(vm, { probeSpice: true });
    if (capabilities.backends.fileTransfer.state !== "connected" || capabilities.backends.spice.state !== "connected") {
      return skip("SPICE channels and file transfer are not all connected");
    }
  ...
  it("returns the expected error when a disposable guest agent is manually disconnected", async ({ skip }) => {
    ...
    if (process.env.BOXES_TEST_AGENT_DISCONNECTED !== "1") {
      return skip("Set BOXES_TEST_AGENT_DISCONNECTED=1 only after manually disconnecting spice-vdagent in the disposable guest");
    }
    await expect(clipboard({ nameOrUuid: vm, operation: "read", selection: "clipboard" }))
